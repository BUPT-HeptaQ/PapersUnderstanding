# evaluate on develop set, and keep the model with the best performance on develop set
# test and translate
...
class encoder_decoder_model(nn.Module):
    ...
    def translate(self, english_matrix, english_matrix_mask, chinese_matrix, hidden, max_length = 20):
        english_matrix_embedded = self.embed_english(english_matrix)
        B, T, embedding_size = english_matrix_embedded.size()
        ...
        prediction = [chinese_matrix]
        for i in range(max_length - 1):
            # print(i)
            # print(chinese_matrix.shape)
            chinese_matrix_embedded = self.embed_chinese(chinese_matrix)

            # print(chinese_embedded.shape)
            # print(h.shape)
            # print(c.shape)

            hiddens, (h, c) = self.decoder(chinese_matrix_embedded, hx=(h, c))
            hiddens = hiddens.contiguous()
            ...
            chinese_matrix = torch.max(decoded, 1)[1].view(B, 1)
            prediction.append(chinese_matrix)

        return torch.cat(prediction, 1)
...
class linear_ND(nn.Module):
    def __init__(self, *args, **kwargs):
        """
        A torch.nn.linear layer modified to accept ND arrays. The function treats the last dimension of
        the input as the hidden dimension
        """
        super(linear_ND, self).__init__()
        self.function = nn.Linear(*args, **kwargs)

    def forward(self, english_matrix):
        size = english_matrix.size()
        output = english_matrix.view(-1, size[-1])
...
def main(args):
    ...
    # encode the words into numbers, encode train and develop sentences into indieces
    ...
    print("start evaluating on develop file")
