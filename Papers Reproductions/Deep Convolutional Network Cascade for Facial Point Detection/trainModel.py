...
def train(level=1):
    ...
    if level > 1 and pool_on:
        ...
        pool.map(runCommand, cmds)  # Map function, apply the runcommand function to each cmds
        pool.close()
        pool.join()
    else:
        for cmd in cmds:
...
if __name__ == '__main__':
    argc = len(sys.argv)  # Get the number of command line strings
    assert (2 <= argc <= 3)
    if argc == 3:
        pool_on = True

    level = int(sys.argv[1])  # the second one behind python
    if 1 <= level <= 3:
        train(level)
    else:
